...
class TransClassifier():
    ...
    def fit_trans_classifier(self, x_train, x_test, y_test, class_name):
        ...
        for epoch in range(self.args.epochs):
            ...
            for i in tqdm(range(0, len(x_train), bs), desc='Training'):
            ...
            # normal_means = all_zs.mean(0, keepdim=True).mean(1, keepdim=True)  # ([5000, 8, 256]) -> ([1, 8, 256]) -> ([1, 1, 256])
            # normal_means = all_zs.mean(0)[0].reshape(1,1,-1)
            ...
            if epoch%2==0 or (epoch + 1 == self.args.epochs):
                ...
                with torch.no_grad():
                    ...
                    print('vaild  Epoch {}/{}\t AUC: {:.8f}'
                        .format(epoch + 1, self.args.epochs,  roc_auc))
        ...
        return best_auc, normal_means

    def test(self, x_test, y_test, normal_means):
